...
- **`workflows/`** -- JavaScript workflow scripts run by the Claude Code `Workflow` tool. One script: `review-fanout.js`, the deterministic dispatch path for `/review` Step 2 -- it applies the canonical dispatch gates, fans the qualifying agents out, and in deep mode puts every finding through a refutation quorum before synthesis. A workflow runs no shell and reads no file; every value it needs is a literal in the script or arrives through `args`.
- **Storage** — Handover files are written to `<project>/.claude/handovers/`.
- **Rules** — `.claude/rules/` contains `skill-rules.md` (hard rules and learned lessons for skills, formerly `command-rules.md`), `review-agent-rules.md`, `cli-overlay-rules.md`, `agent-capability-rules.md`, `workflow-rules.md`, and `readme-structure.md`.
...
- `workflows/` is registered only in `.claude-plugin/plugin.json` (`"workflows": ["./workflows/"]`), and the `Workflow` tool that executes it exists only on Claude Code. Cursor, Codex, Gemini, and OpenCode carry no `workflows` key and no equivalent primitive, so the directory ships to them and does nothing, and `/review --workflow` prints one line and completes on the prompt fan-out. That is why the prompt path stays in `skills/review/SKILL.md` rather than being replaced by the script.
- `node --check` on a workflow file reports a SyntaxError on correct code. The body ends in a top-level `return`, which the workflow runtime requires and Node's module goal rejects. The reported error names a real line, so the obvious fix is to delete the `return` the runtime needs -- that is the trap, not the false failure itself. The syntax gate for `workflows/*.js` is `claude plugin validate ./ --strict` in marketplace-manifest mode; WF6 in `.claude/rules/workflow-rules.md` is the rule.
