...
- **`workflows/`** -- JavaScript workflow scripts run by the Claude Code `Workflow` tool. One script: `review-fanout.js`, the deterministic dispatch path for `/review` Step 2 -- it applies the canonical dispatch gates and fans the qualifying agents out, returning their findings unchanged for Step 3 to synthesize. A workflow runs no shell and reads no file; every value it needs is a literal in the script or arrives through `args`.
