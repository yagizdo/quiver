...
6. Phase 5 invokes the `commit` and `create-pr` skills, gating each action with `AskUserQuestion`. The 5b question offers `Review first -- /quiver:review` ahead of the PR button; picking it delegates to `/quiver:review` and then re-asks 5b without that button.
